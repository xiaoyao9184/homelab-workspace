...
do
    ...
    then
        date=$(date '+%Y-%m-%d')
        groupname="${date}${prefix_delimiter}${name}"
    fi

    if [[ "$suffix_date" == "yes" ]]
    then
        date=$(date '+%Y-%m-%d')
        groupname="${name}${suffix_delimiter}${date}"
    fi
    ...
    docker run \
        --rm \
        -v ${repo}:/git-projects/default \
        -v ${output}:/output-reports/default \
        -e GITSTATS_CONFIG_PROJECT_NAME=${name} \
